...
export class AccountPage {
  ...
  changeFile(event: any, isFront: boolean): void {
    this.ready = false;
    const file: Blob = event.target.files[0];
    if (!file) {
      return;
    }
    this.readFile(this.uploadFile.bind(this), file, isFront);
  }

  private readFile(callback: Function, file: any, isFront: boolean) {
    let reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onload = () => {
      const file_base64 = new String(reader.result).split(",");
      callback(this.toBlob(file_base64[1]), isFront);
    };
  }
